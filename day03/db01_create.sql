...
-- ON DELETE CASCADE	부모 테이블의 데이터가 삭제되면 자식 테이블의 해당 데이터도 자동 삭제
-- ON DELETE RESTRICT	부모 테이블의 데이터가 삭제되지 않도록 제한
-- ON DELETE SET NULL	부모 테이블의 데이터가 삭제되면 자식 테이블의 외래 키 값을 NULL로 설정
-- ON DELETE NO ACTION	삭제 시 아무 작업도 하지 않음 (기본값, RESTRICT와 동일한 동작)
