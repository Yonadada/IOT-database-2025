...
- Visual Studio Code에서 MySQL 연동
    - 확장 > MySQL 검색
        ...
        - Weijan Chen이 개발한 Database Client 설치(추천)
            - 데이터베이스 아이콘 생성
        - Oracle에서 개발한 MySQL Shell for VS Code를 사용이 불편함.. 
...
- SQL 기초
    - 기본 데이터 타입
        - 데이터베이스에는 엄청 많은 데이터 타입이 존재(데이터의 사이즈 저장 용량을 절약하기 위해서)

        - 주요 데이터 타입
            - SmallInt(2byte) - 65535가지 수(음수포함)를 저장(-32767)
            - **Int(4)** - 모든 데이터 타입의 기준, 42억 정수(음수)를 저장 
            - BigInt(8) - Int 보다 더 큰 수를 저장할 때 사용
            - Float(4) - 소수점 아래 7자리까지 저장
            - Decimal(5~17byte) - Float보다 더 큰 수 저장시 사용

            - Char(n) - n은 가변(1~255). 고정길이 문자열 
                - 주의점! Char(10) 에 Hello 글자를 입력하면 **'Hello    '** 와 같이 저장된다!

            - Varchar(n) -n(1~65535). 가변길이 문자열
                - 주의점! Varchar(10)에 Hello를 입력하면 **'Hello'** 저장됨 
            
            - LongText(최대 4GB) - 뉴스나 영화스크립트 저장할 때 사용
            - LongBlob(최대 4GB) - MP3,MP4 음악, 영화데이터 자체를 저장할 때 사용
            - Date(3byte) - 2025-02-27 (현재 오늘날짜) 까지 저장하는 타입 
    ...
    - 테이블 생성 후 확인  
        ...
        2. 연결은 패스
        3. Select Schemas to Reverse Engineer에서 특정 DB를 체크
        4. Execute 버튼 클릭 > next > finish
        5. ERD 확인
